...
export const tauriAPI = {
  ...
  convert: {
    ...
    onProgress: (callback: (progress: ProcessingProgress) => void): (() => void) => {
      ...
      void listen<ProcessingProgress>("convert:progress", (event) => {
    ...
    onComplete: (callback: (result: ProcessingResult) => void): (() => void) => {
      ...
      void listen<ProcessingResult>("convert:complete", (event) => {
        callback(event.payload);
      }).then((fn) => {
        unlisten = fn;
      });
    ...
    onError: (callback: (error: { message: string }) => void): (() => void) => {
      ...
      void listen<{ message: string }>("convert:error", (event) => {
        callback(event.payload);
      }).then((fn) => {
        unlisten = fn;
      });
  ...
  utils: {
    getPathForFile: (file: File) => {
      ...
      if ((file as File & { path?: string }).path) {
        return (file as File & { path?: string }).path as string;
      }
